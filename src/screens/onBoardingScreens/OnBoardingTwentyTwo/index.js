...
const OnBoardingTwentyTwo = props => {
  ...
  // useEffect(() => {
  //   dispatch(setFormData({ hearingSelection: selectedOptions }));
  // }, [selectedOptions]);
    ...
    <SafeAreaView style={styles.safeArea}>
        <View style={styles.container}>
          ...
          <View style={{ marginTop: height * 0.01 }}>
            {optionsArray.map(option => (
              <TouchableOpacity key={option.id} onPress={() => toggleOption(option.id)}>
                <SlecteldBox
                  title={option.text}
                  style={{ backgroundColor: isSelected(option.id) ? '#009379' : '#1D1D1D' }}
                />
              </TouchableOpacity>
            ))}
          </View>
      ...
      <View style={{ alignItems: 'center' , top:70}}>
        <Button
          loading={loading}
          title={`Next`}
          style={{ backgroundColor: selectedOptions.length > 0 ? COLORS.greenText : 'rgba(0, 95, 73, 0.3)' }}
          press={() => handleNext()}
          textStyle={{ color: selectedOptions.length > 0 ? COLORS.heading : 'gray' }}
        />
      </View>
    </SafeAreaView>
  );
